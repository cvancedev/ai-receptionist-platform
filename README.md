...
Sprint 3 through Sprint 7 are certified complete. Milestone 8.1 adds a technology-neutral application preflight for one exact fictional start or resume turn. It composes the certified activated configuration, pinned conversation recovery, Progress Engine, and derived handoff boundaries while processing no message, changing no turn state, and authorizing no response release or external action. The ordinary prototype remains fixture-backed, the durable activated path remains opt-in without fixture fallback, and Milestone 8.2 is Not Started. See the [Sprint 8 Plan](docs/SPRINT_8_PLAN.md), [End-to-End Application Contract](docs/END_TO_END_APPLICATION_CONTRACT.md), and [Sprint 8 Test Plan](docs/SPRINT_8_TEST_PLAN.md).
