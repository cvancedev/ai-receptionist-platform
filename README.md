...
Sprint 0 established the foundation at `v0.0.1`, and Sprint 1 certified the customer-validation website at `v0.1.0`. Sprint 2 defined customer discovery and platform architecture. Sprints 3 through 8 are certified complete. The certified internal fictional MVP composes exact activated configuration and grounded knowledge, deterministic multi-turn intake, atomic durable turns, restart-safe subordinate message and execution evidence, bounded read models, and derived handoff while preserving application authority. Sprint 9.1 defines the controlled-evaluation boundary, Milestones 9.2–9.5 add disconnected runtime, identity, PostgreSQL-readiness, and operational-evidence gates, Milestone 9.6 hardens the existing fictional UI/session, and Milestone 9.7 adds bounded validation evidence and a recommendation-only release gate. The current gate is `NOT_READY` because required manual accessibility and usability reviews have not occurred. Milestone 9.8 is Not Started. No evaluation execution, real AI provider, production database connection, authentication, protected data, external integration, telemetry export, customer response release, or production customer experience has been implemented.
...
Sprints 3 through 8 are certified complete. Milestone 9.1 defines one candidate for a later moderated local/test evaluation using fictional or synthetic scenarios only. Milestones 9.2–9.6 establish the disconnected runtime, identity, persistence-readiness, operational-evidence, and hardening gates. Milestone 9.7 adds bounded validation evidence and a deterministic recommendation-only release gate; its current result is `NOT_READY` pending actual manual accessibility and usability evidence. Controlled evaluation execution, production, protected data, telemetry export, release, and external actions remain unauthorized. Milestone 9.8 is Not Started. See the [Sprint 9 Plan](docs/SPRINT_9_PLAN.md), [Sprint 9.7 Validation Evidence and Release Gate](docs/CONTROLLED_VALIDATION_EVIDENCE_AND_RELEASE_GATE.md), [Sprint 9.6 Hardening Evidence](docs/SPRINT_9_6_HARDENING.md), and [Controlled-Evaluation Boundary](docs/CONTROLLED_EVALUATION_BOUNDARY.md).
