...
Sprint 3, Sprint 4, Sprint 5, and Sprint 6 are certified complete. Sprint 6 preserves technology-neutral application contracts while adding separate opt-in PostgreSQL state and journal adapters, atomic expected-revision state-and-audit coordination, restart-safe fictional integration, explicit recovery and failure outcomes, tenant isolation, and ordered migration-history enforcement. Conversation State remains authoritative, the journal remains audit evidence only, and PostgreSQL remains infrastructure. The ordinary prototype remains synchronous, deterministic, in-memory, and database-independent. Raw or unvalidated output has no authority, customer release remains false, and no production database connection, real provider, external action, or production authentication exists. Sprint 7 has not started. See [Sprint 6 Certification](docs/certification/SPRINT6_CERTIFICATION.md).
