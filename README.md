...
Sprint 3 through Sprint 7 are certified complete. Milestone 8.1 adds a technology-neutral application preflight for one exact fictional start or resume turn. Milestone 8.2 extends it with immutable transient context containing the exact pinned Business Profile, Conversation State revision, explicitly untrusted current customer input, and only activation-bound eligible knowledge. Application-owned grounding validation requires exact versioned source provenance and grants no customer release. No message content is persisted, and no task or transition executes. The ordinary prototype remains fixture-backed, the durable activated path remains opt-in without fixture fallback, and Milestone 8.3 is Not Started. See the [Sprint 8 Plan](docs/SPRINT_8_PLAN.md), [End-to-End Application Contract](docs/END_TO_END_APPLICATION_CONTRACT.md), and [Sprint 8 Test Plan](docs/SPRINT_8_TEST_PLAN.md).
