...
Sprint 0 established the foundation at `v0.0.1`, and Sprint 1 certified the customer-validation website at `v0.1.0`. Sprint 2 defined customer discovery and platform architecture. Sprints 3 through 8 are certified complete. The certified internal fictional MVP composes exact activated configuration and grounded knowledge, deterministic multi-turn intake, atomic durable turns, restart-safe subordinate message and execution evidence, bounded read models, and derived handoff while preserving application authority. Sprint 9.1 defines the controlled-evaluation boundary, Milestone 9.2 adds a disconnected fail-closed server runtime-configuration and secret-reference preflight, Milestone 9.3 preserves the conditional protected-data gate, Milestone 9.4 completes operational PostgreSQL readiness with live disposable backup/restore evidence, Milestone 9.5 adds disconnected bounded operational evidence, privacy, retention, and incident policy, and Milestone 9.6 hardens the existing fictional UI/session against overlapping or unbounded input, retained-state growth, inaccessible status, and internal-error disclosure. Milestone 9.7 is Not Started. No evaluation execution, real AI provider, production database connection, authentication, protected data, external integration, telemetry export, customer response release, or production customer experience has been implemented.
...
Sprint 9.4 operational requirements and the isolated recovery drill are in
[Operational PostgreSQL Readiness](docs/OPERATIONAL_POSTGRESQL_READINESS.md).
...
npm run verify:operational-postgresql-readiness
npm run verify:operational-postgresql-backup-restore
...
- [Operational PostgreSQL readiness](docs/OPERATIONAL_POSTGRESQL_READINESS.md)
- [Operational evidence, privacy, retention, and incidents](docs/OPERATIONAL_EVIDENCE_PRIVACY_AND_INCIDENTS.md)
...
Sprints 3 through 8 are certified complete. Milestone 9.1 defines one candidate for a later moderated local/test evaluation using fictional or synthetic scenarios only. Milestone 9.2 establishes exact environment identity and safe preflight failure. Milestone 9.3 preserves explicit protected-data denial. Milestone 9.4 adds certified migration-source integrity and a passing disposable backup/restore and recovery proof. Milestone 9.5 adds disconnected, in-memory, non-authoritative operational evidence and fail-closed incident decisions. Milestone 9.6 adds bounded reliability, accessibility, performance, usability, and error-sanitization hardening to the existing fictional MVP only. Controlled evaluation, production, protected data, telemetry export, release, and external actions remain unauthorized. Milestone 9.7 is Not Started. See the [Sprint 9 Plan](docs/SPRINT_9_PLAN.md), [Sprint 9.6 Hardening Evidence](docs/SPRINT_9_6_HARDENING.md), [Operational Evidence, Privacy, Retention, and Incidents](docs/OPERATIONAL_EVIDENCE_PRIVACY_AND_INCIDENTS.md), and [Controlled-Evaluation Boundary](docs/CONTROLLED_EVALUATION_BOUNDARY.md).
