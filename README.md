...
Sprint 0 established the foundation at `v0.0.1`, and Sprint 1 certified the customer-validation website at `v0.1.0`. Sprint 2 defined customer discovery and platform architecture. Sprints 3 through 5 are certified complete with the deterministic fictional prototype, provider-neutral mock AI foundation, one controlled in-memory transition, immutable read and audit boundaries, and application-owned progress decisions. Sprint 6 is certified complete. Sprint 7.3 adds opt-in durable PostgreSQL Business Profile and Knowledge Record draft versions with exact reads, while lifecycle transitions and activation remain absent. Milestone 7.4 has not started. AI remains advisory and cannot mutate state directly or release customer content. No real AI provider, production database connection, authentication, external integration, or production customer experience has been implemented.
...
Sprint 3 through Sprint 6 remain certified complete. Sprint 7.3 adds ordered migration 004 and an opt-in PostgreSQL Knowledge Record version repository for immutable draft creation and exact business/profile/record/version reads. It adds no lifecycle transition, approval, activation, active-configuration resolution, knowledge retrieval, conflict resolution, update/delete, production authentication, public administration, provider integration, external action, or end-to-end MVP work. Existing fixtures remain the ordinary prototype default. Milestone 7.4 has not started. See the [Sprint 7 Plan](docs/SPRINT_7_PLAN.md).
