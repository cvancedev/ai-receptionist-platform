...
No real provider/model/SDK, production prompt/schema, public API, production database connection, authentication, billing, or delivery integration exists. Sprints 4 through 7 are certified complete. Milestones 8.1 and 8.2 define and verify the internal application preparation boundary for one exact fictional turn, including activated configuration provenance, durable profile-version scope, deterministic progress, derived handoff readiness, bounded transient activated context, exact grounded-source validation, and explicit no-release authority. Message content is labeled untrusted inside transient context and is not persisted. The ordinary prototype remains fixture-backed, the durable path remains opt-in without fallback, Milestone 8.3 is Not Started, and no public administration, production authentication, provider integration, transition execution, external action, customer release, migration, or dependency change exists. See [End-to-End Application Contract](END_TO_END_APPLICATION_CONTRACT.md), [Sprint 8 Storage Decision](SPRINT_8_STORAGE_DECISION.md), and [Sprint 8 Test Plan](SPRINT_8_TEST_PLAN.md).
