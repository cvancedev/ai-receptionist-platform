...
No real provider/model/SDK, production prompt/schema, public API, production database connection, authentication, billing, or delivery integration exists. Sprints 4 through 7 are certified complete. Sprint 7 proves an opt-in fictional path that reconstructs one exact activated Business Profile and its bound Knowledge Record versions, preserves durable profile-version pinning, completes the application-owned configuration lifecycle without direct SQL state seeding, and fails safely after suspension or during recovery. Sprint 8.0 planning now defines how those certified boundaries may be composed into an internal fictional End-to-End MVP. The ordinary prototype remains fixture-backed, Milestone 8.1 is Not Started, and no public administration, production authentication, provider integration, external action, customer release, or Sprint 8 implementation exists. See [Sprint 7 Certification](certification/SPRINT7_CERTIFICATION.md), [Sprint 8 Plan](SPRINT_8_PLAN.md), and [Sprint 8 Test Plan](SPRINT_8_TEST_PLAN.md).
