...
No real provider/model/SDK, production prompt/schema, public API, production database connection, authentication, billing, or delivery integration exists. Sprints 4 through 7 are certified complete. Milestone 8.1 defines and verifies the internal application preparation boundary for one exact fictional turn, including activated configuration provenance, durable profile-version scope, deterministic progress, derived handoff readiness, and explicit no-release authority. It does not process or persist message content. The ordinary prototype remains fixture-backed, the durable path remains opt-in without fallback, Milestone 8.2 is Not Started, and no public administration, production authentication, provider integration, external action, customer release, migration, or dependency change exists. See [End-to-End Application Contract](END_TO_END_APPLICATION_CONTRACT.md), [Sprint 8 Storage Decision](SPRINT_8_STORAGE_DECISION.md), and [Sprint 8 Test Plan](SPRINT_8_TEST_PLAN.md).
