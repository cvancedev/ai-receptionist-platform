...
No real provider/model/SDK, production prompt/schema, public API, production database connection, authentication, billing, or delivery integration exists. See [AI Integration Prototype Foundation](AI_INTEGRATION_PROTOTYPE_FOUNDATION.md), [Sprint 4 Plan](SPRINT_4_PLAN.md), [Sprint 4 Certification](certification/SPRINT4_CERTIFICATION.md), and [Sprint 5 Certification](certification/SPRINT5_CERTIFICATION.md). Sprint 4 and Sprint 5 are certified complete. Sprint 6.0 defines the Durable Persistence Foundation and selects PostgreSQL architecturally. Milestones 6.1 through 6.4 implement persistence contracts, opt-in PostgreSQL state and journal adapters, and atomic coordination. Milestone 6.5 adds an opt-in fictional restart demonstration that reloads authoritative state directly from scoped Conversation State storage, treats the journal as audit evidence only, and continues deterministic progression from the recovered revision. Milestone 6.6 verifies explicit persistence failure, rollback, restart, isolation, malformed-record, and unsupported-schema behavior without changing production source. The default prototype remains in-memory, and no Sprint 6.7 or production persistence integration exists.
