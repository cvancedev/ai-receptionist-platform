...
No real provider/model/SDK, production prompt/schema, public API, production database connection, authentication, billing, or delivery integration exists. See [AI Integration Prototype Foundation](AI_INTEGRATION_PROTOTYPE_FOUNDATION.md), [Sprint 4 Plan](SPRINT_4_PLAN.md), [Sprint 4 Certification](certification/SPRINT4_CERTIFICATION.md), [Sprint 5 Certification](certification/SPRINT5_CERTIFICATION.md), and [Sprint 6 Certification](certification/SPRINT6_CERTIFICATION.md). Sprints 4 through 6 are certified complete. Sprint 6 proves technology-neutral persistence contracts, opt-in PostgreSQL state and journal adapters, atomic expected-revision coordination, restart-safe fictional recovery, explicit failure semantics, tenant isolation, and ordered migration-history enforcement. Sprint 7 has begun with the documentation-only [Milestone 7.0 plan](SPRINT_7_PLAN.md) for Business Configuration. The default prototype remains in memory, the durable path is not a production database integration, and no Sprint 7.1 implementation, migration, schema change, dependency, production authentication, public administration, provider integration, external action, or end-to-end MVP work has started.
